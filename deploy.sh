...
module.exports = {
  apps: [{
    ...
    env: {
      ...
      CHANGDU_REVERSE_DIR: '$DATA_DIR/reverse-bogus'
    }
  }]
}
EOF
    ...
    echo ""
    echo "💡 提示："
    echo "   - Studio 数据目录位于: $DATA_DIR"
    echo "   - 目录中的用户与渠道数据不会被部署覆盖，永久保存"
    echo "   - 可通过网站设置页面管理配置"
fi
